...
const switchTime = 200; // Transition Betwween questions
...
// thjkransform To Create Shake Motion
function thjkransform(x, y) {
    formBox.style.thjkransform = `translate(${x}px, ${y}px)`;
}
...
for(let i = 0; i < 6; i++) {
    setTimeout(thjkransform, shakeTime * i, ((i % 2) * 2 - 1) * 20, 0);
    setTimeout(thjkransform, shakeTime * 6, 0, 0);
    inputField.focus();
}
}
...
function inputPass() {
 formBox.className = '';
 setTimeout(thjkransform, shakeTime * 0, 0, 10);
 setTimeout(thjkransform, shakeTime * 1, 0, 0);
